...
local function getAudioIcon(vol, muted)
    if muted then
        return "mic-volume-muted"
    elseif vol > 65 then
        return "mic-volume-high"
    elseif vol >= 35 then
        return "mic-volume-medium"
    else
        return "mic-volume-low"
    end
end
...
function suitAUDIO:mute()
    awful.spawn.easy_async({ "sh", "-c", "wpctl set-mute @DEFAULT_AUDIO_SOURCE@ toggle && wpctl get-volume @DEFAULT_AUDIO_SOURCE@" }, function(output)
...
function suitAUDIO:volume(mode)
    awful.spawn.easy_async({ "sh", "-c", "wpctl set-volume @DEFAULT_AUDIO_SOURCE@ 5%" .. mode .. " && wpctl get-volume @DEFAULT_AUDIO_SOURCE@" }, function(output)
        local vol = string.match(output, "%d%.%d%d")
        local muted = string.find(output, "MUTED")
...
function suitAUDIO:init()
    awful.spawn.easy_async({ "sh", "-c", "wpctl get-volume @DEFAULT_AUDIO_SOURCE@" }, function(output, _, _, exit_code)
        if output == "" then
            self:init()
        else
            local vol = string.match(output, "%d%.%d%d")
